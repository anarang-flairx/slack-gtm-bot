...
- CRITICAL routing for deals:
  ...
  • create_company_deal may first ask which deal *pipeline* (if HubSpot has more than one) via a numbered list — show it and wait for one number. Then it requires a deal stage AND a company lifecycle stage; show those numbered lists and ask for two numbers (e.g. "1 3" = deal stage #1, lifecycle #3). When they reply with numbers, immediately call create_company_deal again with pipeline_id / stage / lifecycle_stage — do not re-ask.
...
- When the user asks to create deals for multiple companies in one message, call create_company_deal once per company and report each result. Ask for deal stage + lifecycle once (numbered lists), then reuse those choices for every company. If a tool returns "Error: …", quote that error to the user — do not invent causes like permissions.
...
• Create a company deal — _"move ColigoMed to deals"_ (I'll ask for deal stage + lifecycle with numbered lists; deal named _Company - FlairX_)
