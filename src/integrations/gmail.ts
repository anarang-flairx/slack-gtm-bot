...
/** List ids of messages sent within the lookback window (cheap; ids only). */
export async function listSentMessageIds(
  newerThanDays = 1,
  max = 50,
  ...
  const list = await gmail.users.messages.list({
    userId: "me",
    q: `in:sent newer_than:${newerThanDays}d`,
    maxResults: max,
  });
  ...
  for (const message of list.data.messages ?? []) {
    if (message.id) {
      out.push({ id: message.id, threadId: message.threadId ?? "" });
    }
  }
  return out;
}

/** Fetch a single sent message with its recipients and body text. */
export async function getSentEmail(messageId: string): Promise<SentEmail> {
  const auth = getOAuthClient();
  const gmail = google.gmail({ version: "v1", auth });
  ...
  return {
    ...
    bodyText: extractPlainText(message.data.payload as GmailPart),
    sentAtMs: message.data.internalDate
